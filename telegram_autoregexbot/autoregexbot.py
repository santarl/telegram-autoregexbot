...
class ConfigManager:
    def __init__(self, config_file="autoregexbot.cfg", secrets_file="secrets.cfg"):
        ...
        self.token = os.environ.get("BOT_TOKEN", self.config.get("secrets", "token", fallback=None))
        
        if not self.token or self.token == "YOUR_BOT_TOKEN":
            logger.critical("No valid bot token found in secrets.cfg or Environment Variables.")
            sys.exit(1)
    ...
    def load_config(self):
        try:
            ...
            # Bot Settings
            self.send_as_reply = self.config.getboolean("bot", "send_as_reply", fallback=True)
            self.mention_user = self.config.getboolean("bot", "mention_user", fallback=True)
            self.enable_delete_button = self.config.getboolean("bot", "enable_delete_button", fallback=True)
            self.delete_allowed = self.config.get("bot", "delete_allowed", fallback="sender_or_admin")
            self.cooldown_seconds = self.config.getfloat("bot", "cooldown_seconds", fallback=2.0)

            # Access Control
            self.access_policy = self.config.get("access", "access_policy", fallback="off").lower()
...
def check_access(update: Update) -> bool:
    ...
    if cfg.allow_chat_types and chat.type not in cfg.allow_chat_types:
        if chat.type == "supergroup" and "group" in cfg.allow_chat_types and "supergroup" not in cfg.allow_chat_types:
            pass 
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cfg.mention_user:
        response_text = f"<a href='tg://user?id={user.id}'>{safe_user_name}</a>: {response_text}"
    ...
    while not sent:
        ...
        except (NetworkError, TimedOut, httpx.ConnectError, httpx.ReadTimeout, httpx.WriteTimeout) as e:
            logger.warning(f"Connection failed (Attempt {attempt}). Retrying in 5s... Error: {e}")
...
async def handle_delete_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if allowed:
        try:
            ...
            await query.answer("Could not delete message (Bot needs Delete permissions).")
    else:
